...
export function TemplatePicker({ selected, onSelect }: { selected: string; onSelect: (id: string) => void }) {
  return <section className="control-section"><div className="section-title"><span>01</span><div><h2>Elegí una plantilla</h2><p>Siete maneras de contar tu historia.</p></div></div><div className="template-grid">{templates.map((item) => <button key={item.id} className={`template-card ${selected === item.id ? "selected" : ""}`} onClick={() => onSelect(item.id)}><span className="template-mark">{item.mark}</span><strong>{item.name}</strong><small>{item.note}</small></button>)}</div></section>;
}
