...
export const SheetPreview = forwardRef<HTMLDivElement, { photos: PhotoItem[]; template: string; background: string; title: string; fit: "cover" | "contain"; selectedId: string | null; onSelect: (id: string) => void; onPosition: (id: string, x: number, y: number) => void; onResize: (id: string, frameWidth: number, frameHeight: number) => void; onSwap: (sourceId: string, targetId: string) => void }>(function SheetPreview({ photos, template, background, title, fit, selectedId, onSelect, onPosition, onResize, onSwap }, ref) {
