...
export function DraggablePhoto({ photo, index, fit, selected, onSelect, onPosition, onResize }: { photo: PhotoItem; index: number; fit: "cover" | "contain"; selected: boolean; onSelect: (id: string) => void; onPosition: (id: string, x: number, y: number) => void; onResize: (id: string, frameWidth: number, frameHeight: number) => void }) {
  const start = useRef({ clientX: 0, clientY: 0, x: 0, y: 0, stageWidth: 1, stageHeight: 1 });
  ...
  return <figure className={`photo-cell cell-${index + 1} shape-${photo.shape} ${selected ? "selected-photo" : ""}`} tabIndex={0} aria-label={`Foto ${index + 1}. Arrastrá o usá las flechas para moverla libremente sobre la hoja.`}
    onPointerDown={(event) => { onSelect(photo.id); dragging.current = true; const stage = event.currentTarget.parentElement!.getBoundingClientRect(); start.current = { clientX: event.clientX, clientY: event.clientY, x: photo.moveX, y: photo.moveY, stageWidth: stage.width, stageHeight: stage.height }; event.currentTarget.setPointerCapture(event.pointerId); }}
    onPointerMove={(event) => { if (!dragging.current) return; onPosition(photo.id, clamp(start.current.x + ((event.clientX - start.current.clientX) / start.current.stageWidth) * 100), clamp(start.current.y + ((event.clientY - start.current.clientY) / start.current.stageHeight) * 100)); }}
    onPointerUp={() => { dragging.current = false; }} onPointerCancel={() => { dragging.current = false; }}
    onKeyDown={(event) => { const d = event.shiftKey ? 5 : 1; if (event.key === "ArrowLeft") onPosition(photo.id, clamp(photo.moveX - d), photo.moveY); if (event.key === "ArrowRight") onPosition(photo.id, clamp(photo.moveX + d), photo.moveY); if (event.key === "ArrowUp") onPosition(photo.id, photo.moveX, clamp(photo.moveY - d)); if (event.key === "ArrowDown") onPosition(photo.id, photo.moveX, clamp(photo.moveY + d)); }}
    ...
    <span className="drag-hint">ARRASTRÁ PARA MOVER</span>
